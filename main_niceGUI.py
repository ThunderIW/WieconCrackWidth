...
NUMERIC_FIELDS = [
    ...
    (
        "concrete_modulus",
        "Concrete modulus E_c [GPa] (0 = auto)",
        34.0,
        0.0,
        "Materials",
    ),
...
def save_dialog(result, values: dict):
    ...
    with ui.dialog().props(animated_dialog) as dialog, ui.card().classes("w-96"):
        ...
        with ui.row().classes(
            f"w-full justify-center items-center no-wrap gap-2 py-2 rounded-lg bg-{tone}"
        ):
            ui.icon("check_circle" if result.ok else "cancel").classes(
                "text-2xl text-white"
            )
        ...
        with ui.column().classes("w-full items-center"):
            ...
            case_name = ui.input(placeholder="Enter File Name").props(
                "autofocus input-class=text-center"
            )
        ...
        def save_to_database():
            ...
            if not name:  # don't create nameless rows
                ...
                return
            InteractionFile().add_new_result_to_crackWidth_table(name, values, result)
            dialog.close()
...
def live_chip():
    ...
    """
    values = {
        fid: el.value for fid, el in inputs.items()
    }  # Collectin the inputs values
    if any(
        values.get(fid) is None for fid, *_ in NUMERIC_FIELDS
    ):  # Here we check if the required fields are filled
    ...
    tone = (
        "positive" if r.ok else "negative"
    )  # Here we are get the value of the result and if it is true then the chip will be green and if it is false then the chip will be red
...
SAVED_COLUMNS = {
    ...
    "wk": {
        "header": "Crack width w_k [mm]",
        "decimals": 3,
        # compared against THIS row's own limit, not a hardcoded number
        "rules": {"cw-fail": "x > data.w_max", "cw-pass": "x <= data.w_max"},
    },
    "w_max": {"header": "Limit w_max [mm]", "decimals": 2},
    "ok": {
        "header": "Pass",
        "rules": {"cw-pass": "x === true", "cw-fail": "x === false"},
    },
    "mode": {"header": "Mode"},
...
async def on_selected_changed():
    ...
    view_more_detail_button.text = (
        f"Select {n} File{'' if n == 1 else 's'} then click to view"
    )
...
def build_detail_pdf(cases: list[dict]) -> bytes:
    ...
    for d in cases:
        ...
        pdf.cell(
            0,
            5,
            f"Saved {d['created_at'][:16].replace('T', ' ')}",
            new_x="LMARGIN",
            new_y="NEXT",
        )
        pdf.ln(3)
        ...
        pdf.set_fill_color(*_PDF_LINE)
        pdf.rect(x0, bar_y, w, bar_h, style="F")
        pdf.set_fill_color(*tone)
        pdf.rect(x0, bar_y, w * min(ratio, 1.0), bar_h, style="F")
        pdf.ln(6)
...
async def get_selected_rows():
    ...
    def export_detail_to_PDF():
        ...
        fname = (
            f"crack_width_detail_{selected[0]['name']}_{stamp}.pdf"
            if len(selected) == 1
            else f"crack_width_detail_{stamp}.pdf"
        )
        ui.download(data, fname)
        ui.notify(
            f"Exported {len(selected)} File{'' if len(selected) == 1 else 's'}",
            type="positive",
            position="center",
            timeout=3,
        )
    ...
    with (
        ui.dialog().props(animated_dialog) as detail_dialog,
        ui.card().classes("w-[560px] max-h-[80vh] overflow-y-auto"),
    ):
        with ui.row().classes("w-full justify-end items-center gap-1"):
            ui.button(icon="picture_as_pdf", on_click=export_detail_to_PDF).props(
                "flat round dense"
        ...
        for d in selected:
            ...
            ui.label(f"Saved {d['created_at'][:16].replace('T', ' ')}").classes(
                "text-xs opacity-60"
            )
            ...
            with (
                ...
                with ui.column().classes("gap-0"):
                    with ui.row().classes("items-baseline gap-1 no-wrap"):
                        ui.label(f"{d['wk']:.3f}").classes(
                            f"text-3xl font-bold text-{tone} leading-none"
                        )
                        ui.label("mm").classes("text-sm opacity-70")
                    ui.label(f"w_k  ·  limit {d['w_max']:.2f} mm").classes(
            ...
            ui.linear_progress(min(ratio, 1.0), show_value=False).props(
                f"color={tone} rounded size=6px"
            ...
            for group, fields in by_group.items():
                with ui.expansion(
                    group, icon=GROUP_ICONS.get(group), value=False
                ).classes("w-full"):
                    for kind, fid, label, default, extra in fields:
                        with ui.row().classes(
                            "w-full justify-between items-baseline no-wrap"
                        ):
                            ui.label(label).classes("text-xs opacity-70 truncate")
                            ui.label(fmt(d[fid])).classes(
                                "text-sm font-bold whitespace-nowrap"
                            )

            with ui.expansion("Results", icon="fact_check", value=True).classes(
                "w-full"
            ):
                for key, label in RESULT_LABELS.items():
                    with ui.row().classes(
...
async def delete_selected_cases():
    ...
    ]  # Here we are getting the ids of the selected cases
    names = " "
    # .join(r["name"] for r in selected) #Here we are getting the names of the selected cases

    def confirm():
        deleted = InteractionFile().delete_results_from_crackWidth_table(
            ids
        )  # Here is where we are actually deleting the cases
        confirm_dialog.close()
        refresh_saved_cases()  # Refresh the grid after deletion to reflect the changes made to the database
        ui.notify(
            f"Deleted {deleted} File{'' if deleted == 1 else 's'}",
            type="warning",
            position="center",
            timeout=3,
        )
    ...
    with ui.dialog().props(animated_dialog) as confirm_dialog, ui.card():
        ui.label(f"Delete {len(ids)} File{'' if len(ids) == 1 else 's'}?").classes(
            "text-lg font-bold"
        )
        ...
        ui.label("This cannot be undone !").classes(
            "text-sm opacity-70 text-red-500 font-bold"
        )
        with ui.row():
...
async def delete_all_cases():
    ...
    with ui.dialog().props(animated_dialog) as confirm_dialog, ui.card():
        ui.label(
            f"Delete ALL ({len(ids)} file{'' if len(ids) == 1 else 's'}) ?"
...
def build_ui() -> None:
    ...
    def apply_theme():
        ...
        """
        primary = (
            "#4a90d9" if dark.value else "#16235c"
        )  # Here we are setting the primary color to a different shade of blue
    ...
    with ui.header().classes("bg-[#16235c] text-white items-center"):
        ...
        def update_Menu_context(e: events.MouseEventArguments) -> None:
            ...
            with (
                context_menu.clear()
            ):  # Clearing the context menu item to make fresh ones
        ...
        with ui.element("div").classes("bg-white rounded-md p-1 shrink-0"):
            ...
            with (
                ui.interactive_image(
                    LOGO_IMAGE, on_mouse=update_Menu_context, events=["contextmenu"]
                )
                .classes("h-16 w-28")
                .props("fit=contain")
            ):
        ...
        """
        with (
            ui.dialog().props(animated_dialog) as credits_dialog,
            ui.card().classes("w-96 items-center"),
        ):
        ...
        with (
            ui.dialog().props("maximized") as video_dialog,
            ui.card().classes(
                "w-full h-full p-2 flex flex-col items-center justify-center"
            ),
        ):
            ui.button(icon="close", on_click=video_dialog.close).props(
                "flat round dense"
            ).classes("self-end")
            ui.video(
                _asset_dir() / "docs" / "videos" / "7-21-2026_VERSION_2.1.7.mp4"
            ).classes("flex-1 w-full min-h-0")

        # This is the dialog window that is used for zooming in the screenshot
        with (
            ui.dialog().props("maximized") as zoom_dialog,
            ui.card().classes(
                "w-full h-full p-2 flex flex-col items-center justify-center"
            ),
        ):
            ui.button(icon="close", on_click=zoom_dialog.close).props(
                "flat round dense"
            ).classes("self-end")  # Here we are closing the dialog
            zoom_image = (
                ui.image().props("fit=contain").classes("flex-1 w-full min-h-0")
            )  # this is where the image is zoomed in

        def open_zoom(src):
            zoom_image.set_source(
                src
            )  # Here we are setting the image source to the image that was clicked on
        ...
        with (
            ui.dialog().props(animated_dialog) as help_dialog,
            ui.card().classes("w-96 items-center"),
        ):
            ...
            ui.button(
                "Watch walkthrough", icon="videocam", on_click=video_dialog.open
            ).props("flat")
            ...
            with ui.column().classes("w-full items-start gap-3 py-2"):
                for i, (title, detail, image) in enumerate(
                    help_steps, start=1
                ):  # Here we are iterating through the HELP_STEPS list
                    with ui.row().classes("w-full items-start no-wrap gap-3"):
                        ...
                        with (
                            ui.avatar(color="primary", text_color="white")
                            .props("size=28px")
                            .classes("shrink-0")
                        ):
                            ui.label(str(i))
                        with ui.column().classes("gap-1"):
                            ...
                            if image and (_asset_dir() / image).exists():
                                ...
                                with (
                                    ui.interactive_image(src)
                                    .props("fit=contain")
                                    .classes("w-full rounded border")
                                ):
                                    ui.button(
                                        icon="zoom_in",
                                        on_click=lambda s=src: open_zoom(s),
                                    ).props("round dense color=primary").classes(
                                        "absolute bottom-1 right-1 m-1 opacity-80"
                                    )
    ...
    with ui.card().classes("w-full h-full p-2 flex flex-col"):
        ...
        with (
            ui.dialog().props(f"maximized {animated_dialog}") as dialog,
            ui.card().classes(
                "w-full h-full p-2 flex flex-col items-center justify-center"
            ),
        ):
            ...
            ui.image(REFERENCE_IMAGE).props("fit=contain").classes(
        ...
        with ui.dialog().props(f"{animated_dialog}") as upload_dialog:
            ...
            async def upload_file(e):
                ...
                try:
                    ...
                    if dest.suffix != ".csv":
                        ui.notify(
                            f"Only .csv is supported (got {dest.suffix or 'no extension'})",
                            type="warning",
                            position="center",
                        )
                        return
                    ...
                    if number_of_rows == 0:
                        ui.notify(
                            "That .csv has no rows to import.",
                            type="warning",
                            position="center",
                        )
                        return
                    ...
                    refresh_saved_cases()
                    ui.notify(
                        f"Imported {saved} case(s) into the system.",
                        type="positive",
                        location="center",
                    )
                    upload_dialog.close()
            ...
            with ui.card():
                ui.button(icon="close", on_click=upload_dialog.close).props(
                    "flat round dense"
                ).classes("self-end")
                ui.label("Upload File for testing").classes(
                    "text-lg font-bold w-full text-center"
                )
                ui.upload(
                    on_upload=upload_file, max_files=1, label="Please upload a .csv"
                ...
                with progress:
                    ui.label().bind_text_from(
                        progress, "value", lambda v: f"{v * 100:.0f} %"
                    )
        ...
        with ui.row().classes("items-center"):
            ...
            with (
                ui.fab("dataset", label="Import and Download", direction="right")
                .props('rounded padding="sm md"')
                .style("min-height: 36px")
            ):
                with ui.fab_action("upload", on_click=upload_dialog.open).props(
                    "rounded"
                ):
                    ui.tooltip(
                        "Upload a .csv, .xls, and .xlsx file to run multiple cases "
                    )

                with ui.fab_action(
                    "download",
                    on_click=lambda: ui.download.file(
                        _asset_dir() / "sample_data" / "crack_cases_template.csv",
                        "crack_cases_template.csv",
                    ),
                ).props("rounded"):
                    ui.tooltip(
                        "Download a sample .csv file to import and run multiple cases "
                    )

        for group, fields in by_group.items():
            with ui.expansion(group, value=False, icon=GROUP_ICONS.get(group)).classes(
                "w-full"
            ):
                with ui.grid(columns=2).classes("w-full"):
                    for kind, fid, desc, default, extra in fields:
                        if kind == "Numeric":
                            inputs[fid] = ui.number(
                                desc, value=default, min=extra, format="%.2f"
                            ).props("debounce=300")
                        else:
        ...
        def wire_creep_field():
            ...
            ui.add_css(
                ".creep-tip.q-field--disabled .q-field__control"
                " { pointer-events: auto; }"
            )
            inputs["creep_coeff"].tooltip(
                "Only used under long-term loads and for the currently selected load duration"
        ...
        with (
            ui.grid(columns=4)
            .classes("w-full overflow-x-auto gap-x-8 gap-y-1")
            ...
            with ui.column().classes("gap-1"):
                ...
                inputs["load_duration"].on_value_change(
                    lambda e: (
                        creep_row.classes(add="line-through opacity-50")
                        if e.value == "short"
        ...
        with ui.column().classes("gap-2"):
            live_chip()
            ui.button("Save File", icon="save_as", on_click=save_result).props(
                "no-caps"
            )
        ...
        with ui.row().classes(
            "w-full justify-center items-center gap-2 action-buttons"
        ):
            delete_button = ui.button(
                "Delete File",
                on_click=delete_selected_cases,
                icon="delete",
                color="negative",
            )
            view_more_detail_button = ui.button(
                "View More Detail",
                on_click=get_selected_rows,
                icon="visibility",
                color="primary",
            )
    ...
    ui.keyboard(
        on_key=lambda e: (
            fullscreen.toggle()
            if (e.action.keydown and not e.action.repeat and e.key == "f")
            else None
        )
    )
    ...
    with ui.page_sticky(x_offset=18, y_offset=18):
        ...
        ui.button(on_click=toggle_dark).props("fab color=primary").bind_icon_from(
            dark, "value", lambda v: "dark_mode" if v else "light_mode"
        )
